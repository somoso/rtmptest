...
class MP3Box {
    static func create(_ data:Data) throws -> MP3Box {
        ...
        case "moov", "trak", "mdia", "minf", "stbl", "edts":
            return MP3ContainerBox(size: size, type: type)
        case "MP3v", "s263", "avc1":
            return MP3VisualSampleEntryBox(size: size, type: type)
        case "mvhd", "mdhd":
            return MP3MediaHeaderBox(size: size, type: type)
        case "MP3a":
            return MP3AudioSampleEntryBox(size: size, type: type)
        case "esds":
            return MP3ElementaryStreamDescriptorBox(size: size, type: type)
        case "stts":
            return MP3TimeToSampleBox(size: size, type: type)
        case "stss":
            return MP3SyncSampleBox(size: size, type: type)
        case "stsd":
            return MP3SampleDescriptionBox(size: size, type: type)
        case "stco":
            return MP3ChunkOffsetBox(size: size, type: type)
        case "stsc":
            return MP3SampleToChunkBox(size: size, type: type)
        case "stsz":
            return MP3SampleSizeBox(size: size, type: type)
        case "elst":
            return MP3EditListBox(size: size, type: type)
        default:
            return MP3Box(size: size, type: type)
        }
    }
    ...
    fileprivate(set) var parent:MP3Box? = nil
    ...
    func getBoxes(byName:String) -> [MP3Box] {
        return []
    }
    ...
    func create(_ data:Data, offset:UInt32) throws -> MP3Box {
        let box:MP3Box = try MP3Box.create(data)
        box.parent = self
...
extension MP3Box: CustomStringConvertible {
...
// MARK: -
class MP3ContainerBox: MP3Box {

    fileprivate var children:[MP3Box] = []
    ...
    override func load(_ file:FileHandle) throws -> UInt32 {
        ...
        while (size != offset) {
            let child:MP3Box = try create(file.readData(ofLength: 8), offset: offset)
    ...
    override func getBoxes(byName:String) -> [MP3Box] {
        var list:[MP3Box] = []
...
// MARK: -
final class MP3MediaHeaderBox: MP3Box {
    var version:UInt8 = 0
...
// MARK: -
final class MP3ChunkOffsetBox: MP3Box {
    var entries:[UInt32] = []
...
// MARK: -
final class MP3SyncSampleBox: MP3Box {
    var entries:[UInt32] = []
...
// MARK: -
final class MP3TimeToSampleBox: MP3Box {
...
// MARK: -
final class MP3SampleSizeBox: MP3Box {
    var entries:[UInt32] = []
...
// MARK: -
final class MP3ElementaryStreamDescriptorBox: MP3ContainerBox {
...
// MARK: -
final class MP3AudioSampleEntryBox: MP3ContainerBox {
    ...
    override func load(_ fileHandle: FileHandle) throws -> UInt32 {
        ...
        let esds:MP3Box = try create(fileHandle.readData(ofLength: 8), offset: offset)
...
// MARK: -
final class MP3VisualSampleEntryBox: MP3ContainerBox {
    ...
    override func load(_ fileHandle: FileHandle) throws -> UInt32 {
        let buffer:ByteArray = ByteArray(data: fileHandle.readData(ofLength: MP3VisualSampleEntryBox.dataSize))
        ...
        var offset:UInt32 = UInt32(MP3VisualSampleEntryBox.dataSize)
        let child:MP3Box = try MP3Box.create(fileHandle.readData(ofLength: 8))
        child.parent = self
...
// MARK: -
final class MP3SampleDescriptionBox: MP3ContainerBox {
    override func load(_ fileHandle: FileHandle) throws -> UInt32 {
        children.removeAll(keepingCapacity: false)
        ...
        let numberOfEntries:UInt32 = try buffer.readUInt32()
        for _ in 0..<numberOfEntries {
            let child:MP3Box = try create(fileHandle.readData(ofLength: 8), offset: offset)
            offset += try child.load(fileHandle)
            children.append(child)
        }
...
// MARK: -
final class MP3SampleToChunkBox: MP3Box {
    struct Entry:CustomStringConvertible {
...
// MARK: -
final class MP3EditListBox: MP3Box {
    struct Entry: CustomStringConvertible {
...
// MARK: -
final class MP3Reader: MP3ContainerBox {
    ...
    func readData(ofBox:MP3Box) -> Data {
...
// MARK: -
final class MP3TrakReader {
    static let defaultBufferTime:Double = 500

    var trak:MP3Box
    var bufferTime:Double = MP3TrakReader.defaultBufferTime
    ...
    init(id:Int, trak:MP3Box) {
        self.id = id
        self.trak = trak

        let mdhd:MP3Box? = trak.getBoxes(byName: "mdhd").first
        if let mdhd:MP3MediaHeaderBox = mdhd as? MP3MediaHeaderBox {
            timeScale = mdhd.timeScale
        }

        let stss:MP3Box? = trak.getBoxes(byName: "stss").first
        if let stss:MP3SyncSampleBox = stss as? MP3SyncSampleBox {
        ...
        let stts:MP3Box? = trak.getBoxes(byName: "stts").first
        if let stts:MP3TimeToSampleBox = stts as? MP3TimeToSampleBox {
            var timeToSample:[MP3TimeToSampleBox.Entry] = stts.entries
            for i in 0..<timeToSample.count {
                let entry:MP3TimeToSampleBox.Entry = timeToSample[i]
        ...
        let stsz:MP3Box? = trak.getBoxes(byName: "stsz").first
        if let stsz:MP3SampleSizeBox = stsz as? MP3SampleSizeBox {
            sampleSize = stsz.entries
        }

        let stco:MP3Box = trak.getBoxes(byName: "stco").first!
        let stsc:MP3Box = trak.getBoxes(byName: "stsc").first!
        var offsets:[UInt32] = (stco as! MP3ChunkOffsetBox).entries
        var sampleToChunk:[MP3SampleToChunkBox.Entry] = (stsc as! MP3SampleToChunkBox).entries
    ...
    func execute(_ reader:MP3Reader) {
        do {
            handle = try FileHandle(forReadingFrom: reader.url)

            if let avcC:MP3Box = trak.getBoxes(byName: "avcC").first {
                delegate?.didSet(config: reader.readData(ofBox: avcC), withID: id, type: AVMediaTypeVideo)
            }
            if let esds:MP3ElementaryStreamDescriptorBox = trak.getBoxes(byName: "esds").first as? MP3ElementaryStreamDescriptorBox {
...
extension MP3TrakReader: TimerDriverDelegate {
...
extension MP3TrakReader: CustomStringConvertible {
    // MARK: CustomStringConvertible
    var description:String {
        return Mirror(reflecting: self).description
    }
